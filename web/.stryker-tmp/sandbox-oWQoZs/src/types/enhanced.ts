...
export type SessionContext = OnboardingContext | DiscoveryContext | CategoryExplorationContext | GiftSelectionContext | ResearchContext;
 ...
 */
export type FiniteString<T extends string> = T extends string ? string extends T ? never : T : never;
 ...
 */
export type OptionalIfUndefined<T, K extends keyof T> = undefined extends T[K] ? Partial<Pick<T, K>> : Required<Pick<T, K>>;
 ...
 * Async function type
 */
export type AsyncFunction<TArgs extends ReadonlyArray<unknown> = [], TReturn = void> = (...args: TArgs) => Promise<TReturn>;
 ...
 */
export type DiscriminatedUnion<T, K extends keyof T, V extends T[K]> = T extends Record<K, V> ? T : never;
