...
interface SecureWelcomeEmailProps {
  email: string
  source?: string
...
/**
 * GDPR-Compliant Secure Email Template for aclue Newsletter
 *
 * Security Features:
 * - Full GDPR Article 13, 21 compliance
 * - CAN-SPAM Act compliance
 * - One-click unsubscribe functionality
 * - Privacy policy and data rights information
 * - Physical address disclosure
 * - Consent tracking and transparency
 * - WCAG AA accessibility compliance
 * - Security headers support
 *
 * Compliance Checklist:
 * ✅ Unsubscribe mechanism (GDPR Art. 21, CAN-SPAM)
 * ✅ Privacy policy link (GDPR Art. 13)
 * ✅ Physical address (CAN-SPAM)
 * ✅ Data retention info (GDPR Art. 13(2)(a))
 * ✅ Data rights info (GDPR Art. 15-22)
 * ✅ Consent reference (GDPR Art. 7)
 * ✅ Third-party disclosure (GDPR Art. 13(1)(e))
 * ✅ Contact information (GDPR Art. 13(1)(a))
 */
function SecureWelcomeEmail({
  email,
  source = 'newsletter',
  subscriptionDate = new Date().toLocaleDateString('en-GB'),
  unsubscribeToken = '{{unsubscribe_token}}',
  preferencesToken = '{{preferences_token}}'
}: SecureWelcomeEmailProps) {

  // Base URLs for compliance links
  const baseUrl = 'https://aclue.app'
  const unsubscribeUrl = `${baseUrl}/unsubscribe?token=${unsubscribeToken}&email=${encodeURIComponent(email)}`
  const preferencesUrl = `${baseUrl}/email-preferences?token=${preferencesToken}&email=${encodeURIComponent(email)}`
  const privacyPolicyUrl = `${baseUrl}/privacy-policy`
  const termsUrl = `${baseUrl}/terms`
  const dataRightsUrl = `${baseUrl}/data-rights`

  // White aclue logo SVG
  const aclueLogoSvg = `
  ...
  const emailStyles = `
    @media (prefers-color-scheme: dark) {
      ...
      .compliance-footer { background: #18181b !important; color: #a1a1aa !important; }
      .compliance-footer a { color: #60a5fa !important; }
    }
    ...
    @media only screen and (max-width: 600px) {
      ...
      .compliance-footer { padding: 20px !important; }
    }
  `
    ...
    <div style={{ fontFamily: 'Arial, sans-serif', lineHeight: 1.6, color: '#333', margin: 0, padding: 0 }}>
      ...
      <div style={container} className="container">
        ...
        <div style={header} className="header">
          ...
          <h1 style={h1}>Welcome to aclue!</h1>
        </div>

        <div style={content} className="content">
          {/* GDPR: Consent Reference */}
          <div style={consentReminder}>
            You're receiving this email because you subscribed to aclue updates on {subscriptionDate}.
            If you didn't subscribe, please <a href={unsubscribeUrl} style={{ color: '#0284c7' }}>click here</a> to unsubscribe.
          </div>
          ...
          <section>
            ...
            <p style={text}>
              aclue is where AI-powered intelligence meets personalised discovery, making gift-giving effortless and meaningful.
            </p>
          </section>
          ...
          <section>
            <h2 style={sectionHeading}>📧 You're Successfully Subscribed</h2>
            <p style={text}>
          ...
          {/* Feature Showcase */}
          <section>
            <h2 style={sectionHeading}>🎁 What Makes aclue Special</h2>
            <div style={glassmorphismCard}>
              <p style={{ ...text, marginBottom: '0' }}>
                ...
                <br/><br/>
                <strong>📱 Seamless Experience:</strong> Browse, save, and organise your gift ideas with an intuitive, beautiful interface.
              </p>
            </div>
          </section>
          ...
          {/* Early Access Badge */}
          <div style={earlyAccessBadge}>
            <p style={{ ...text, marginBottom: '8px', fontWeight: '600', color: '#075985' }}>
              ⭐ Early Subscriber
            </p>
            <p style={{ ...text, marginBottom: '0', fontSize: '14px', color: '#3f3f46' }}>
              You're among the first to experience aclue's intelligent gifting platform
            </p>
          </div>
          ...
          <div style={footer}>
            ...
            <p style={{ marginBottom: '8px' }}>
              <strong>The aclue Team</strong>
            </p>
            <p style={{ marginBottom: '16px' }}>
              <em>AI-powered gifting platform</em>
            </p>
            <p style={{ marginBottom: '0' }}>
              <a href="https://aclue.app" style={{ color: '#0284c7', textDecoration: 'none' }}>
                Visit aclue.app
              </a>
            </p>
          </div>
        ...
        {/* GDPR-Compliant Footer Section */}
        <div style={complianceFooter} className="compliance-footer">
          {/* CRITICAL: Unsubscribe Links (GDPR Art. 21, CAN-SPAM) */}
          <div style={{ marginBottom: '20px' }}>
            ...
            <p style={{ margin: '0' }}>
              <a href={unsubscribeUrl}
                 style={{ color: '#0284c7', textDecoration: 'underline', marginRight: '12px' }}>
                Unsubscribe
              </a>
              |
              <a href={preferencesUrl}
                 style={{ color: '#0284c7', textDecoration: 'underline', marginLeft: '12px' }}>
                Update Preferences
              </a>
            </p>
          </div>

          {/* CRITICAL: Privacy & Legal Links (GDPR Art. 13) */}
          <div style={{ marginBottom: '20px' }}>
            <p style={{ margin: '0 0 8px 0', fontWeight: '600' }}>
              Legal & Privacy
            </p>
            <p style={{ margin: '0' }}>
              ...
              |
              <a href={termsUrl}
                 style={{ color: '#0284c7', textDecoration: 'underline', marginLeft: '12px', marginRight: '12px' }}>
                Terms of Service
              </a>
              |
              <a href={dataRightsUrl}
                 style={{ color: '#0284c7', textDecoration: 'underline', marginLeft: '12px' }}>
                Your Data Rights
              </a>
            </p>
          ...
          {/* Data Protection Information (GDPR Art. 13(2)) */}
          <div style={{ marginBottom: '20px', fontSize: '12px', color: '#9ca3af' }}>
            <p style={{ margin: '0 0 8px 0', fontWeight: '600' }}>
              Data Protection
            </p>
            <p style={{ margin: '0' }}>
              We process your data to provide our newsletter service based on your consent.
              Your data is stored securely and retained until you unsubscribe.
          ...
          {/* CRITICAL: Physical Address (CAN-SPAM Act) */}
          <div style={{ marginBottom: '20px' }}>
            <p style={{ margin: '0 0 8px 0', fontWeight: '600' }}>
              Contact Information
            </p>
            <p style={{ margin: '0' }}>
              ...
              London, EC1A 1BB<br/>
              United Kingdom<br/>
              <a href="mailto:privacy@aclue.app" style={{ color: '#0284c7' }}>
                privacy@aclue.app
              </a>
            </p>
          </div>
          ...
          <div style={{ fontSize: '12px', color: '#9ca3af' }}>
            <p style={{ margin: '0' }}>
              Data Controller: aclue Limited |
              DPO Contact: <a href="mailto:dpo@aclue.app" style={{ color: '#0284c7' }}>dpo@aclue.app</a> |
...
// Export component
export default function SecureEmailPreviewPage() {
  return <SecureWelcomeEmail
